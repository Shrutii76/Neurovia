...
const SymbolDetectiveGame = () => {
  ...
  const symbols = [
    ...
    {
      symbol: "-",
      options: ["Multiply", "Subtract", "Add"],
      correctIndex: 1
    },
    {
      symbol: "×",
      options: ["Divide", "Add", "Multiply"],
      correctIndex: 2
    },
    {
      symbol: "÷",
      options: ["Divide", "Subtract", "Equal"],
      correctIndex: 0
    },
    {
      symbol: "=",
      options: ["Greater than", "Equal", "Less than"],
      correctIndex: 1
    },
    {
      symbol: ">",
    ...
    {
      symbol: "<",
      options: ["Less than", "Greater than", "Equal"],
      correctIndex: 0
    },
    {
      symbol: "≠",
      options: ["Equal", "Not equal", "Greater than"],
      correctIndex: 1
    }
  ];
 ...
 useEffect(() => {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-pink-300 via-purple-300 to-blue-300 overflow-hidden relative">
        ...
        {floatingTreats.map((treat, index) => (
          <div
            key={index}
            className="absolute text-4xl floating-treat"
            style={{
              top: treat.top,
              left: treat.left,
              right: treat.right,
        ...
        <div className="flex items-center justify-center min-h-screen p-4">
          <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-2xl p-8 max-w-md w-full text-center">
            ...
            <button
              onClick={startGame}
              className="bg-purple-500 hover:bg-purple-600 text-white text-xl font-bold py-4 px-8 rounded-2xl transform hover:scale-105 transition-all duration-200 shadow-lg"
            >
              Find Symbols! 🔍
            </button>
          </div>
  ...
  if (gameState === 'results') {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-pink-300 via-purple-300 to-blue-300 overflow-hidden relative">
        {/* Floating Treats with celebration animation */}
        {floatingTreats.map((treat, index) => (
          <div
            key={index}
          ...
          <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-2xl p-8 max-w-md w-full text-center">
            <div className="mb-6">
              ...
              <p className="text-xl text-blue-600">
                You solved {percentage}% of the symbols!
              </p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <button
                onClick={resetGame}
                className="bg-purple-500 hover:bg-purple-600 text-white text-xl font-bold py-4 px-8 rounded-2xl transform hover:scale-105 transition-all duration-200 shadow-lg w-full"
              >
                Play Again! 🔄
              </button>
            </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-300 via-purple-300 to-blue-300 overflow-hidden relative">
      ...
      {floatingTreats.map((treat, index) => (
        <div
          key={index}
          className="absolute text-3xl floating-treat"
          style={{
            top: treat.top,
            left: treat.left,
            right: treat.right,
      ...
      <div className="relative z-10 p-4">
        ...
        <div className="w-full bg-white/30 rounded-full h-3 mb-6">
          <div
            className="bg-purple-500 rounded-full h-3 transition-all duration-500 ease-out"
            style={{ width: `${((currentQuestion + 1) / symbols.length) * 100}%` }}
          ></div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center px-4">
        <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-2xl p-6 max-w-2xl w-full">
          <div className="text-center mb-8">
            ...
            <div className="bg-gray-50 border-4 border-purple-200 rounded-2xl p-12 mb-8 mx-auto max-w-sm">
              <div className="text-8xl font-bold text-purple-600">
                {symbols[currentQuestion].symbol}
              </div>
            </div>
          </div>
            ...
            <div className="text-center mt-6">
              ...
              {selectedAnswer !== symbols[currentQuestion].correctIndex && (
                <p className="text-gray-600 mt-2">
                  The correct answer is: <strong>{symbols[currentQuestion].options[symbols[currentQuestion].correctIndex]}</strong>
                </p>
              )}
            </div>
          )}
          ...
          <div className="text-center mt-6 text-gray-500">
            Symbol {currentQuestion + 1} of {symbols.length}
          </div>
        </div>
      </div>
    </div>
